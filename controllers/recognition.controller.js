...
const addRecognizedEmployee = async (req, res) => {
  try {
    ...
    if (!employee_id || !award_type || !period || !period.year) {
      return res.status(400).json({
        success: false,
        message: "employee_id, award_type and period (with year) are required",
      });
    }

    if (!allowedAwardTypes.includes(award_type)) {
      return res.status(400).json({
        success: false,
        message:
          "Invalid award type. Must be one of: " + allowedAwardTypes.join(", "),
      });
    }
    ...
    let _id = "123x";
    if (award_type === "Performer of the Month") {
      const uploadedFiles = [];
      // upload all given files
      for (const file of files) {
        const uploadedFile = await uploadToServer(
          file,
          `uploads/recognition/${period.year}/${newRecognition._id}`
        );
      ...
      if (uploadedFiles.length > 0) {
        newRecognition.files = uploadedFiles;
      }
    }
    ...
    res.status(200).json({
      ...
      recognition_id: savedRecognition._id,
    });
  } catch (error) {
